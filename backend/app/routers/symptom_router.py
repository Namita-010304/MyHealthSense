...
async def create_symptom(symptom: SymptomCreate, db: AsyncSession = Depends(get_db)):
...
async def get_symptoms(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Symptom).order_by(Symptom.created_at.desc())
    )
...
async def update_symptom(symptom_id: int, updated_data: SymptomCreate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Symptom).filter(Symptom.id == symptom_id)
    )
...
async def delete_symptom(symptom_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Symptom).filter(Symptom.id == symptom_id)
    )
    symptom = result.scalar_one_or_none()
